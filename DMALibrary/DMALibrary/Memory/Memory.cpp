...
bool Memory::FixCr3()
{
	...
	while (true)
	{
		...
		if (nt == VMMDLL_STATUS_SUCCESS && atoi((LPSTR)bytes) == 100)
			break;
		LOG("[-] Current Plugin Loading progress: %i\n", atoi((LPSTR)bytes));
...
uintptr_t Memory::AOBScan(BYTE* pbPattern)
{
	BYTE subsetPattern[] = {
	...
	DWORD subsetSize = sizeof(subsetPattern);
	...
	{
		ctxSearch.search[ctxSearch.cSearch].cb = subsetSize;
		memcpy(ctxSearch.search[ctxSearch.cSearch].pb, subsetPattern, subsetSize);
		ctxSearch.cSearch++;
	}
	...
	bool result = VMMDLL_MemSearch(mem.vHandle, mem.current_process.PID, &ctxSearch, &pvaSearchResult, &cvaSearchResult);

	if (result)
	{
		for (DWORD i = 0; i < cvaSearchResult; i++)
		{
			BYTE buffer[84];
			DWORD bytesRead;
			if (VMMDLL_MemReadEx(mem.vHandle, mem.current_process.PID, pvaSearchResult[i], buffer, 84, &bytesRead, VMMDLL_FLAG_NOCACHE) && bytesRead == 84)
			{
				if (memcmp(buffer, pbPattern, 84) == 0)
				{
					auto fucky = pvaSearchResult[i];
					VMMDLL_MemFree(pvaSearchResult);
					return fucky;
				}
			}
		}
...
uintptr_t Memory::AOBScanVad(BYTE* pbPattern)
{
	PVMMDLL_MAP_VAD vads;
	if (!VMMDLL_Map_GetVadW(mem.vHandle, mem.current_process.PID, true, &vads))
		return false;

	for (int i = 0; i < vads->cMap; i++)
	{
		auto vad = vads->pMap[i];
		auto size = vad.vaEnd - vad.vaStart + 1;
		BYTE* buffer = new BYTE[size];
		if (!VMMDLL_MemReadEx(mem.vHandle, this->current_process.PID, vad.vaStart, (PBYTE)buffer, size, NULL, VMMDLL_FLAG_NOCACHE))
			continue;
		for (size_t i = 0; i < size; i++)
		{
			if (memcmp(buffer + i, pbPattern, 84) == 0)
			{
				auto fucky = vad.vaStart + i;
				return fucky;
			}
		}
		delete buffer;
