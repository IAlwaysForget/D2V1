...
LRESULT WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	if (size.x != previous_size.x || size.y != previous_size.y)
	{
		...
		if (fuser.GetType() == EType::TYPE_DX11)
		{
		...
		//::PostMessage(hWnd, WM_SIZE, 0, MAKELPARAM(size.x, size.y));
		//Change window size
	}
...
void Fuser::Run(void (*callback)())
{
	if (!m_is_initialized)
	{
		printf("[!] Fuser is not initialized\n");
		return;
	}
...
void setWindowTarget(std::string game_name, ImVec2 game_size, HWND m_hwnd)
{
	while (true)
	{
		auto game_hwnd = FindWindowA(game_name.c_str(), 0);
		...
		else
		{
			...
			game_size.x = game_rect.right - game_rect.left;
			game_size.y = game_rect.bottom - game_rect.top;
			...
			MoveWindow(m_hwnd, game_rect.left, game_rect.top, game_size.x, game_size.y, true);
			...
			if (hwnd_active == game_hwnd)
			{
				...
				SetWindowPos(m_hwnd, hwndtest, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
			}
		}
	}
}
...
bool Fuser::Initialize(ImVec2 window_size)
{
	...
	ctx.m_logger->Info("Creating window of size %.2f %.2f\n", window_size.x, window_size.y);

	m_hwnd = ::CreateWindowEx(NULL, L"awoo", L"awoo", WS_POPUP | WS_VISIBLE, 0, 0, window_size.x, window_size.y, nullptr, nullptr, wc.hInstance, nullptr);

	if (!m_hwnd)
	{
		ctx.m_logger->Error("Failed to create window\n");
		return false;
	}
	...
	{
		ctx.m_logger->Error("[!] Failed to setup DirectX\n");
		CleanupDeviceD3D();
		::UnregisterClassW(wc.lpszClassName, wc.hInstance);
		return 1;
	}
